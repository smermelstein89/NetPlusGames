#!/usr/bin/env python3
"""
CIDR Flash — Level 2: CIDR → Mask
---------------------------------
Given a CIDR prefix (e.g. /24), identify the full subnet mask (255.255.255.0).
"""

import ipaddress, json, os, random, time

SCOREFILE = "cidr_flash_scores.json"
MODE_KEY = "lv2_cidr2mask"
...
def print_highscores():
    ...
    print(f"\n🏆 High-Scores — CIDR Flash Lv2 (CIDR → Mask) 🏆")
    if not scores:
        print("  (none yet)")
        return
...
def get_mem_tip(cidr, mask):
    block = 2 ** (32 - cidr)
    return f"Correct: {mask}  |  {32 - cidr} host bits → block {block}"

# ---------- Core Quiz ----------
def run_quiz(name, rounds=10):
    print("\n🎯 CIDR Flash Lv2 — CIDR → Mask Conversion")
...
# ---------- Main ----------
def main():
    print("=== 🧮 CIDR Flash Lv2 — CIDR → Mask ===")
