#!/usr/bin/env python3
"""
Subnet Snap — Smart Learning Path (Level 2)
-------------------------------------------
Level 2: recognize CIDR <-> Mask for /16–/23.
 - Multiple choice questions
 - Visual block hints (third octet changes)
...
def load_progress():
    ...
    return {"current_level": 2, "best_streak": 0, "high_score": 0}
...
def block_ranges_third_octet(cidr):
    """Return list of (start,end) ranges for the third octet."""
    size = block_size_from_cidr(cidr) // 256  # divide by 256 because 4th octet fully host
    ranges, start = [], 0
    while start < 256:
...
def visual_hint(cidr):
    ...
    size = block_size_from_cidr(cidr) // 256
    subs = block_ranges_third_octet(cidr)
    shown = subs[:8]
    ranges_text = "\n  ".join([f"{a:>3}–{b:<3}" for a,b in shown]) + ("  ..." if len(subs) > 8 else "")
    return (
        f"\n💡 Hint for /{cidr}\n"
        f"Mask: {mask}\n"
        f"Changing octet: 3rd\n"
        f"Block size in 3rd octet: {size}\n"
        f"Subnet ranges (3rd octet):\n  {ranges_text}\n"
        f"Network addresses start at .{shown[0][0] if shown else 0}.0 "
        f"then jump by {size} in the 3rd octet.\n"
    )
...
def main():
    prog=load_progress()
    if prog.get("current_level",1)>2:
        print("📈 You already cleared Level 2 previously. (Progress loaded)")
    print("=== 🧮 Subnet Snap — Level 2 (/16–/23) ===")
    print("Focus: third-octet masks (bigger networks). Get 10 correct in a row to level up.\n")
    ...
    while True:
        ...
        if streak>=TARGET_STREAK:
            print("\n🏆 LEVEL UP! You nailed 10 in a row at Level 2!")
            prog["current_level"]=max(prog.get("current_level",2),LEVEL+1)
            break
    ...
    if prog["current_level"]>2:
        print("🎉 Next time we can unlock Level 3 (/8–/15).")
